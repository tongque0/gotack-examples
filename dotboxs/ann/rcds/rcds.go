...
*     Creation Date       :     [2014-04-29 22:14]
*     Last Modified       :     [2014-05-11 22:25]
*     Description         :
**********************************************************************************/
...
const (
    numHashBlock = 73
    hashSize     = 13000000 / numHashBlock
    hashExpire   = hashSize * numHashBlock / 3 * 2
)
...
func (self *Hash) NewHashKey(b *board.Board) *HashKey {
    return &HashKey{H: b.H, V: b.V, S0: b.S[b.Now], S1: b.S[b.Now^1]}
}

func (self *Hash) Query(b *board.Board) (win int8, ok bool) {
    val := self.GetHashValue(self.NewHashKey(b))
    if val != nil {
        win, ok = b.Now, true
        if val.Z == -1 {
            win = b.Now ^ 1
...
func (self *Hash) Add(b *board.Board, win int8) {
    var z int8 = -1
    if win == b.Now {
        z = 1
    }
    self.InsertHashValue(self.NewHashKey(b), b.Turn, z)
}
...
func (self *Hash) GetHashValue(k *HashKey) *HashValue {
    ...
    if v, ok := self.HashTable[idx][*k]; ok {
        ...
        atomic.StoreUint64(&v.Stamp, self.GetStamp())
        return v
    }
    self.RWMutex[idx].RUnlock()
    return nil
...
func (self *Hash) InsertHashValue(k *HashKey, turn, z int8) *HashValue {
    idx := int(k.H^k.V) % numHashBlock
    val := &HashValue{Z: z, Turn: turn, Stamp: self.GetStamp()}
...
func (self *Hash) GetStamp() uint64 {
    return atomic.AddUint64(&self.TimeStamp, 1)
}
