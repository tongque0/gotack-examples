...
import (
	...
	"gotack/dotbot/ann"
	"gotack/dotbot/ann/qeval"
	"gotack/dotbot/ann/qrcds"
	"log"
	"math"
	"math/rand"
	"runtime"
...
var (
	...
	annTurn, leafturn            int
	ucb_C, ucb_FPU               [2]float64
	timelimit_l, timelimit_h     float64
	hash                         qrcds.Hash
	annsChan                     = make(chan *qeval.Anns, 1024)
)
...
func AdjustUCB(b *qboard.QBoard, timeout *uint) {
	...
	if annTurn, leafturn = 28+b.Now, 30+b.Now; b.Turn < 11 {
		annTurn, leafturn = 30+b.Now, 32+b.Now
	}
...
func (self *QUCTANN) GetName() string {
	return string("QUCTANN")
}

func (self *QUCTANN) MakeMove(b *qboard.QBoard, timeout uint, verbose bool) (h, v int32, err error) {
	if b.Turn <= 1 {
		hash.ClearAll()
		runtime.GC()
	}
	...
	if verbose {
		defer func() {
			log.Println("Turn:", b.Turn, ", Elapse:", time.Since(enterTime).String(), ", Sim:", sumSimulation,
				...
				", searchMove:", searchMove, ", annTurn:", annTurn, ", leafturn:", leafturn,
	...
	for t := 0; t < numThread; t++ {
		...
		go func() {
			...
			for len(stop) == 0 {
				...
				rleafturn := leafturn + int(rand.Intn(2))
...
func Sim(b *qboard.QBoard, anns *qeval.Anns) bool {
	...
	if b.Turn >= annTurn+rand.Intn(2) || b.Turn >= 36-searchMove+rand.Intn(2) {
		resm, best, now := ms[0], -INF, b.Now
		var tmpinput [25]ann.Type
		for _, i := range rand.Perm(len(ms)) {
			m := ms[i]
			b.Move(m)
			mm := b.Play(tmppms[:])
...
func WinRate(b *qboard.QBoard, anns *qeval.Anns) (r0, r1 float64) {
	if ie := b.IsEnd(); ie != 0 {
		if ie < 0 {
			r0, r1 = 1, 0
		} else {
