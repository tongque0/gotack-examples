...
func (b *NoGoBoard) IsGameOver() bool {
	...
	// 		if cell == 1 { // 假设1代表黑棋
	// 			blackCount++
	// 		} else if cell == 2 { // 假设2代表白棋
	// 			whiteCount++
	// 		}
	// 	}
	// }
	// // 检查棋子数量是否合理
	// if whiteCount > blackCount || blackCount > whiteCount+1 {
	// 	return true // 游戏结束，因为棋子数量不合理
	// }
	// // 检查棋盘是否已满
	// if blackCount+whiteCount == len(b.Board)*len(b.Board[0]) {
	// 	return true // 游戏结束，因为棋盘已满
	// }
	return false
}
...
func (b *NoGoBoard) Clone() gotack.Board {
	...
	for x := range b.Board {
		for y := range b.Board[x] {
			cloned.Board[x][y] = b.Board[x][y]
		}
	}

	// 复制访问状态数组
	for x := range b.dfs_air_visit {
		for y := range b.dfs_air_visit[x] {
			cloned.dfs_air_visit[x][y] = b.dfs_air_visit[x][y]
		}
	}
